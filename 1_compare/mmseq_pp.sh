INPUT=$1
START=$(date +%s.%N)
FASTA_FOLDER="/work/idoerg/hchung/pamfun2/data/protein_fa"
OUTPUT_FOLDER="/work/idoerg/hchung/pamfun2/compare/data/mmseqs_results/$INPUT"
...
if [ ! -f "$OUTFILE" ]; then
    ...
    /work/idoerg/hchung/mmseqs/bin/mmseqs easy-search -s 7.5 -v 0 --k-score 85 -e 10000.0 --max-seqs 4000 --format-output "query,target,bits" $FILE"temp".match compare/mmseqsDB/balanced_assemblyDB "$OUTPUT_FOLDER/${INPUT}_S7_5.mmseq" compare/mmseqsDB/balanced_assemblyDB_tmp
    rm $FILE"temp".match
    rm $FILE"temp"
else
